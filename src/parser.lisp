...
(defun ys-buf-push (ys c)
  ...
  (let ((buf (ys-buf ys)))
    (declare (type (array character (*)) buf))
...
(defun %json-string (text i)
  (declare (type simple-string text) (type fixnum i))
  (let ((n (length text)))
    (unless (and (< i n) (char= (schar text i) #\"))
                 ...
                 (case e
                   ...
                   (#\u
                    ...
                    (let ((code 0))
                      (dotimes (k 4)
                        (incf i)
                        (let ((d (digit-char-p (schar text i) 16)))
                          (unless d
                            (return-from %json-string (values nil nil)))
                          (setf code (+ (* code 16) d))))
...
(defun %json-number (text i)
  ...
  (let ((n (length text))
        (start i))
    (when (and (< i n) (char= (schar text i) #\-))
      (incf i))
    (when (>= i n)
    ...
    (let ((c (schar text i)))
      (cond
        ...
        ((char<= #\1 c #\9)
         (loop do (incf i)
               while (and (< i n) (digit-char-p (schar text i)))))
        (t (return-from %json-number (values nil nil)))))
    (let ((realp nil))
      (when (and (< i n) (char= (schar text i) #\.))
        (incf i)
        (unless (and (< i n) (digit-char-p (schar text i)))
          (return-from %json-number (values nil nil)))
        (setf realp t)
        (loop do (incf i)
              while (and (< i n) (digit-char-p (schar text i)))))
      (when (and (< i n) (or (char= (schar text i) #\e) (char= (schar text i) #\E)))
        (incf i)
        (when (and (< i n) (or (char= (schar text i) #\+) (char= (schar text i) #\-)))
          (incf i))
        (unless (and (< i n) (digit-char-p (schar text i)))
          (return-from %json-number (values nil nil)))
        (setf realp t)
...
(defun try-json-fast-path (text)
  ...
  (let* ((text (if (simple-string-p text) text (copy-seq text)))
         (i 0)
         (n (length text)))
    (declare (type simple-string text) (type fixnum i n))
    (when (and (plusp n) (char= (schar text 0) #\ufeff))
      (setf i 1))
    (setf i (%json-ws text i))
    (when (>= i n)
